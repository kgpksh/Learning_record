...
con.close()
con=sqlite3.connect('D:/Documents/CODING/Binance_RL_Bot/3rd_cost.db')
...
for epoch in range(1,120):
    ...
    for minute in range(1,299340):
        ...
        #넥스트 프라이스가 아니라 현재 가격으로 맞추고 조정해볼것
        if ep>0 :
    ...
    if epoch%print_interval==0 and epoch!=0:
            ...
            torch.save(q,'D:/Documents/CODING/Binance_RL_Bot/models/model_all_3/{}'.format(str(total_value)+'_'+str(epoch)+'.pt'))
            torch.save(q.state_dict(),'D:/Documents/CODING/Binance_RL_Bot/models/model_save_use_3/{}'.format(str(total_value)+'_'+str(epoch)+'.pt'))
            torch.save(
                {'model_state_dict': q.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),}
                ,'D:/Documents/CODING/Binance_RL_Bot/models/model_checkpoint_3/{}'.format(str(total_value)+'_'+str(epoch)+'.tar')
            )
con.close