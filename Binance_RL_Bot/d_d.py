...
con.close()
# con=sqlite3.connect('D:/Documents/CODING/Binance_RL_Bot/4th_cost.db')
class Qnet(nn.Module):
    def __init__(self):
        super(Qnet, self).__init__()
        self.fc1 = nn.Linear(2162, 4096)
...
for epoch in itertools.count():
    ...
    for minute in range(1,292140):
        ...
        # if ep>0 :
        #     next_state=supplyer(minute,current_ratio,(next_price-ep)*cur_eth_amount*1000/total_value)
        # elif ep<0:
        #     next_state=supplyer(minute,current_ratio,(-ep-next_price)*cur_eth_amount*1000/total_value)
        # elif ep==0:
    ...
    if epoch%print_interval==0 and epoch!=0:
            ...
            torch.save(q,'D:/Documents/CODING/Binance_RL_Bot/models/model_all_4/{}'.format(str(total_value)+'_'+str(epoch)+'.pt'))
            # torch.save(q.state_dict(),'D:/Documents/CODING/Binance_RL_Bot/models/model_save_use_4/{}'.format(str(total_value)+'_'+str(epoch)+'.pt'))
            torch.save(
                {'model_state_dict': q.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'epoch' : epoch}
                ,'D:/Documents/CODING/Binance_RL_Bot/models/model_checkpoint_4/{}'.format(str(total_value)+'_'+str(epoch)+'.tar')
            )